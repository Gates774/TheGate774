...
- In constitutional_basis, do not provide only a section number or Schedule label. Give the exact section, article, or Schedule item, then include a short direct quotation from the available constitutional source when the wording is present. Follow the quotation with a plain-English explanation of how it supports the conclusion or action.
- For every materially relevant statute, regulation, institutional rule, or policy, give its exact title and year, relevant section or provision, and a short direct quotation when the wording is present in the retrieved source. Explain the legal effect, consequence, remedy, duty, or protection in plain English.
- Never reconstruct or guess quotation wording. If the exact provision is not present in the available sources, provide only a clearly labelled paraphrase and state that the exact text should be verified from the official source.
- Use a compact format that remains readable in the existing report: "Authority — Section/Provision: \"short source quotation\". Meaning: plain-English explanation. Application: why it supports this action."
...
Run all three steps (Classify -> Identify Tier -> Produce Actionable Report). Before writing the report, identify all materially relevant laws and institutional frameworks in the retrieved sources, including constitutional provisions, specific sections or provisions, short source quotations where available, consequences or remedies, and parallel responsible authorities where supported. Use the Civic Action Guide for civic routing and the retrieved sources for specific legal foundations. In constitutional_basis, include the exact constitutional citation, a short source-grounded quotation or clearly labelled paraphrase, and its plain-English meaning. In rationale, action_plan, documents_needed, escalation_path, rights_reminder, and contact, identify additional laws or institutional frameworks with their sections, quotations or labelled paraphrases, consequences, and responsible routes. Do not invent or reconstruct wording. Put all legal foundations into the existing fields without adding JSON keys. Substitute the residence State into "Governor of <State>" and the residence LGA into "Chairman of <LGA> Local Government Area" when those tiers apply. Reply with STRICT JSON only, matching the schema in the system prompt.`;
    ...
    // Keep the existing JSON schema, but attach the actual retrieved source
    // passages to rationale so the frontend can display them after Subject of
    // the matter. This prevents the source evidence from being lost after the
    // AI returns its structured report.
    if (retrievedLegalPassages && parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
      const report = parsed as Record<string, unknown>;
      const existingRationale = typeof report.rationale === "string"
        ? report.rationale.trim()
        : "";
      report.rationale = [
        existingRationale,
