...
function parseMdaCsv(csv: string): MdaSource[] {
  ...
  return lines.slice(1).map((line) => {
    ...
    return {
      ...
      routingTerms: profile.routingTerms,
      jurisdiction: (/local|lga|area council|municipal|council/i.test(row.category ?? "") ? "local" : /state/i.test(row.category ?? "") ? "state" : /national/i.test(row.category ?? "") ? "national" : "federal") as MdaSource["jurisdiction"],
...
const MANDATE_PROFILES: Array<{
...
}> = [
  {
    ...
    routingTerms: ["defective goods", "faulty product", "misrepresented goods", "consumer complaint", "refund", "warranty", "unfair trade"],
    excludeTerms: ["election", "voter registration", "polling unit", "ballot", "police misconduct", "crime investigation"],
  },
  {
    pattern: /independent national electoral commission|\binec\b/i,
    mandate: "Electoral registration, elections, voter services, and election administration.",
    aliases: ["INEC", "election", "voter registration", "polling unit", "electoral"],
    issueTerms: ["election", "electoral", "voter", "voting", "polling", "ballot", "result", "candidate", "party"],
    routingTerms: ["election", "electoral", "voter registration", "polling unit", "ballot", "election result"],
    excludeTerms: ["laptop", "computer", "defective goods", "refund", "seller", "purchase", "consumer complaint", "hospital treatment", "police misconduct"],
  },
  {
    ...
    routingTerms: ["robbery", "theft", "kidnapping", "criminal investigation", "police protection", "immediate danger", "emergency"],
  },
  {
    ...
    routingTerms: ["police brutality", "police misconduct", "complaint against police officer", "discipline of police", "police abuse"],
  },
  {
    ...
    routingTerms: ["money laundering", "financial crime", "economic crime", "fraud", "corruption"],
  },
  {
    ...
    routingTerms: ["abuse of office", "bribery by public official", "corrupt practice", "public corruption"],
  },
  {
    ...
    routingTerms: ["fake drug", "counterfeit drug", "adulterated drug", "expired drug", "unregistered medicine", "fake medicine", "counterfeit medicine", "unsafe food product"],
    excludeTerms: ["doctor negligence", "hospital negligence", "wrong diagnosis", "nurse misconduct", "hospital refused treatment"],
  },
  {
    ...
    routingTerms: ["road accident", "traffic offence", "road safety", "dangerous driving", "vehicle registration"],
  },
  {
    ...
    routingTerms: ["human rights violation", "torture", "unlawful detention", "rights violation", "discrimination"],
  },
];

function authorityProfile(institution: string, category: string): { mandate: string; aliases: string[]; issueTerms: string[]; routingTerms: string[] } {
...
const ROUTING_TERM_WEIGHTS: Record<string, number> = {
  "police brutality": 100,
  "police misconduct": 100,
  "complaint against police officer": 100,
  "police abuse": 100,
  "fake drug": 100,
  "counterfeit drug": 100,
  "adulterated drug": 100,
  "expired drug": 100,
  "unregistered medicine": 100,
  "fake medicine": 100,
  "counterfeit medicine": 100,
  "unsafe food product": 100,
  "defective goods": 90,
  "faulty product": 90,
  "misrepresented goods": 90,
  "consumer complaint": 80,
  "complaint against police": 90,
  "money laundering": 90,
  "financial crime": 80,
  "economic crime": 80,
  "road accident": 70,
  "human rights violation": 90,
};

function routingTermScore(source: MdaSource, query: string): number {
  const value = normalize(query);
  const knownProfile = MANDATE_PROFILES.find((item) => item.pattern.test(source.institution));
  if (!knownProfile) return 0;
  return knownProfile.routingTerms.reduce((total, term) => {
    if (!value.includes(normalize(term))) return total;
    return total + (ROUTING_TERM_WEIGHTS[normalize(term)] ?? 25);
  }, 0);
}

function jurisdictionScore(source: MdaSource, query: string): number {
  const value = normalize(query);
  const wantsLocal = /local government|lga|area council|council chairman|ward|municipal/.test(value);
  const wantsState = /state ministry|state government|governor|commissioner/.test(value);
  if (wantsLocal && source.jurisdiction === "local") return 25;
  if (wantsState && source.jurisdiction === "state") return 20;
  if ((wantsLocal && source.jurisdiction === "state") || (wantsState && source.jurisdiction === "local")) return -20;
  return 0;
}
...
function authorityScore(source: MdaSource, query: string): number {
  const value = normalize(query);
  const knownProfile = MANDATE_PROFILES.find((item) => item.pattern.test(source.institution));
  const profileHits = knownProfile?.issueTerms.filter((term) => value.includes(normalize(term))).length ?? 0;
  if (knownProfile && profileHits === 0) return 0;
  const excludedHits = knownProfile?.excludeTerms?.filter((term) => value.includes(normalize(term))).length ?? 0;
  if (knownProfile && excludedHits > 0) return 0;
  ...
  const isOrdinaryCrime = /\b(robbery|armed robbery|theft|stealing|kidnapping|crime|criminal assault|missing person)\b/.test(value);
...
export async function searchMdaDirectory(
  ...
  const scored = sources
    .map((source) => {
      ...
      const routingScore = routingTermScore(source, queryText);
