...
interface StepCarouselProps {
  ...
  className?: string;
  /** Deprecated: kept for backward-compat. Auto-advance is disabled. */
          ...
          {steps.map((s, i) => {
            ...
            return (
              <button
                ...
                disabled={!clickable}
                className={cn(
                  "group relative z-10 flex flex-col items-center gap-2 min-w-0",
                  clickable ? "cursor-pointer" : "cursor-not-allowed",
                )}
              >
                <span
                  className={cn(
                    "h-8 w-8 shrink-0 rounded-full flex items-center justify-center text-xs font-semibold transition-all duration-300 ring-1 bg-background",
                    active &&
