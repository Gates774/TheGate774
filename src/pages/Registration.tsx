...
import { REGISTRATION_CATEGORIES } from "@/data/registrationCategories";
...
export default function Registration() {
  ...
  const category = useMemo(
    () => REGISTRATION_CATEGORIES.find((c) => c.id === categoryId) ?? null,
    [categoryId],
  );
  ...
  const submit = async () => {
    if (!category || !subcategory) {
      toast.error("Please choose a category and the specific registration.");
      return;
    }
    setLoading(true);
    setAnalysis(null);
    ...
    const content = [
      `Registration requested: ${subcategory.label} (${category.label}).`,
      subcategory.hint ? `Registering authority: ${subcategory.hint}.` : "",
    ...
    try {
      ...
      if (!data?.ok || !data?.analysis) throw new Error("No registration guide returned");
      setAnalysis(data.analysis as ComplaintAnalysis);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Could not build your registration guide");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-secondary/40 via-background to-background">
      <Helmet>
        <title>Register for a Scheme or ID — THE GATE®</title>
        <meta
          name="description"
          content="Register for a Nigerian government scheme, ID or programme — NIN, BVN, PVC, RSA, NHIA, CAC and more — guided by THE GATE®."
        />
        <link rel="canonical" href="https://thegate774app.lovable.app/registration" />
      </Helmet>

      <ModuleHeader eyebrow="Module 06 · Registration" title="Register for a Scheme" />

      <section className="container max-w-5xl py-10 space-y-10">
        {!analysis && (
          <>
            <Step number={1} label="Choose a registration category">
            ...
            {category && (
              <Step number={2} label={`Pick the specific registration in ${category.label}`}>
              ...
              <Step number={3} label="Your location & any specifics">
                ...
                <div className="mt-4 animate-fade-in">
                  ...
                  <Textarea
                    ...
                    rows={4}
                    placeholder="E.g. 'First-time NIN enrolment, adult, no birth certificate yet — what proof of identity is acceptable?'"
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pt-5 mt-4 border-t border-border/60">
                  <p className="text-xs text-muted-foreground">
                    We'll list the enrolment centre, documents, fees and timelines.
                  </p>
                  <Button
                    onClick={submit}
                    disabled={!canSubmit}
                  ...
                  >
                    ...
                    {loading ? "Building your guide…" : "Build my registration guide"}
                  </Button>
                </div>
              </Step>
            )}
        ...
        {analysis && (
          <div className="space-y-4 animate-fade-in">
            <ComplaintReportCard analysis={analysis} eyebrow="Your Registration Guide" />
            <div className="flex justify-end">
              <Button variant="outline" onClick={reset} className="rounded-xl gap-2">
                <RotateCcw className="h-4 w-4" /> Start another registration
              </Button>
            </div>
          </div>
        )}
